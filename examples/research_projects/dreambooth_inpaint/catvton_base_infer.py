...
def run_inference(
    ...
    **kwargs
    ...
    # 打印推理参数，重点是用哪些latent
    print(f"使用latent: {latent_append_num}")
    if condition_image is not None:
        print(f"使用condition_image")
    if canny_image is not None:
        print(f"使用canny_image")
    if cloth_warp_image is not None:
        print(f"使用cloth_warp_image")
    if openpose_image is not None:
        print(f"使用openpose_image")
    # 把模型移动到cuda
    unet.to(device)
    vae.to(device)
...
def run_inference_2(
    ...
    **kwargs
    ...
    if use_warp_as_condition: # 用warp作为条件而不是底图
        latents_to_concat = [masked_real_images_latents, condition_latents, masked_part_latents]
