...
from train_grid_extra import DreamBoothDataset, collate_fn, adapt_unet_with_catvton_attn
...
def run_inference_on_test_dataset(test_data_root):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(test_dataloader, desc="处理测试数据")):
            ...
            # 加载image_encoder
            image_encoder = CLIPPeftModel.from_pretrained(base_model_path=CONFIG["pretrained_clip_model_path"],
                                                        finetune_path=CONFIG["pretrained_image_encoder_path"],
            ...
            result = run_inference_2(
                ...
                predict_together=CONFIG["predict_together"]
            )
